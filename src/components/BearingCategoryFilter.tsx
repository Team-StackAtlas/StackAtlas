    ...
    <div className="px-4 pb-3">
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {BEARING_CATEGORIES.map(item => {
          ...
          return (
            <button key={item.name} type="button" onClick={() => onCategoryChange(item.name)} className={cn('flex min-h-24 items-center gap-3 rounded-2xl border bg-white p-4 text-left transition-all dark:bg-zinc-900', selected ? 'border-emerald-300 ring-2 ring-emerald-100 dark:border-emerald-500/50 dark:ring-emerald-500/10' : 'border-slate-200 hover:border-slate-300 dark:border-zinc-800 dark:hover:border-zinc-700')}>
      ...
      {category && (
        <div className="mt-3 rounded-2xl border border-slate-200 bg-white p-3 dark:border-zinc-800 dark:bg-zinc-900">
          <div className="mb-2 flex items-center justify-between gap-3"><p className="text-sm font-semibold text-slate-700 dark:text-zinc-300">Filter within {category.name}</p><button type="button" onClick={onReset} className="inline-flex items-center gap-1 text-sm font-medium text-emerald-600 hover:underline dark:text-emerald-400"><X size={14}/> Reset</button></div>
