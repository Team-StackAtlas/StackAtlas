...
import { SUPPLEMENTS, type Post } from '../data/mockData';
...
/** Desktop-only right rail (x.com-style): trending bearings and a few
 * substances to discover. Keeps the wide viewport from reading as a lone
 * narrow feed column floating in empty space. */
export default function SquareRail({ posts }: { posts: Post[] }) {
  const { isFollowing, toggleFollow } = useFollowing();
  const bearings = topBearings(posts, 8);
  const suggestions = SUPPLEMENTS.filter((s) => !isFollowing('substance', s.id)).slice(0, 4);
              ...
              <div key={s.id} className="flex items-center gap-2.5 rounded-xl px-1 py-1.5">
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-sm font-semibold text-slate-900 dark:text-zinc-100">{s.name}</span>
                    <span className="flex items-center gap-1 text-xs text-slate-500 dark:text-zinc-500">
                      <AccessBadge classification={s.classification} />
                      {s.classification}
                    </span>
                  </span>
                </Link>
