...
export default function Admin() {
  ...
  const load = async () => {
    ...
    setMessage('');
    try {
      const [queue, audit] = await Promise.all([services.moderation.listQueue(), services.moderation.listLog()]);
      setItems(queue);
      setLog(audit);
  ...
  useEffect(() => { void load(); }, [services, tab]);
  ...
  const act = async (item: ModerationQueueItem, status: 'reviewed' | 'rejected' | 'action_taken' | 'approved') => {
    if (!services) return;
    try {
  ...
  const userAction = async (target: ProfileDTO, status: string) => {
    if (!services) return;
    try {
      if (status === 'site_admin' || status === 'user') await services.moderation.setSiteRole(target.id, status);
  ...
  return <div className="mx-auto max-w-6xl space-y-5 px-4 pb-12 pt-6">
    ...
    {tab === 'suggest' && <QueueTable items={suggestions} onAct={act} actions={['reviewed', 'approved', 'rejected']} />}
...
function QueueTable({ items, actions, onAct }: { items: ModerationQueueItem[]; actions: string[]; onAct: (item: ModerationQueueItem, status: never) => void }) {
  return <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900"><div className="overflow-x-auto"><table className="min-w-full divide-y divide-slate-200 text-sm dark:divide-zinc-800"><thead><tr className="text-left text-xs uppercase text-slate-500"><th className="px-4 py-3">ID</th><th className="px-4 py-3">Target</th><th className="px-4 py-3">Reporter/Submitter</th><th className="px-4 py-3">Reported user</th><th className="px-4 py-3">Reason/Field</th><th className="px-4 py-3">Note / Suggestion</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Dates</th><th className="px-4 py-3">Actions</th></tr></thead><tbody className="divide-y divide-slate-100 dark:divide-zinc-800">{items.map((item) => <tr key={`${item.submissionType}-${item.id}`}><td className="px-4 py-3 font-mono text-xs">{item.id}</td><td className="px-4 py-3"><strong>{item.targetType}</strong><br />{item.targetLabel ?? item.targetId}</td><td className="px-4 py-3">{item.username ? `@${item.username}` : 'Unknown'}</td><td className="px-4 py-3">{item.reportedUsername ? `@${item.reportedUsername}` : '—'}</td><td className="px-4 py-3">{item.reason ?? item.targetField ?? '—'}</td><td className="max-w-xs px-4 py-3">{item.preview || '—'}</td><td className="px-4 py-3">{item.status}</td><td className="px-4 py-3 text-xs">{new Date(item.createdAt).toLocaleString()}<br />{item.updatedAt ? new Date(item.updatedAt).toLocaleString() : ''}</td><td className="px-4 py-3"><div className="flex flex-wrap gap-1">{actions.map((action) => <button key={action} onClick={() => onAct(item, action as never)} className="rounded-lg bg-slate-100 px-2 py-1 text-xs font-semibold dark:bg-zinc-800">{action.replace('_', ' ')}</button>)}</div></td></tr>)}{items.length === 0 && <tr><td colSpan={9} className="px-4 py-10 text-center text-slate-500">No records.</td></tr>}</tbody></table></div></div>;
}

function Empty({ title, text }: { title: string; text: string }) { return <section className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900"><h2 className="font-bold">{title}</h2><p className="mt-1 text-sm text-slate-500">{text}</p></section>; }
