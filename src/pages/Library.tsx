...
function SavedRow({ item, albums, albumItems, onUnsave, onAdd, onRemove }: { item: HookSavedItem & { unavailable?: boolean }; albums: { id: string; title: string }[]; albumItems: { id: string; albumId: string; savedItemType: SavedItemType; savedItemId: string }[]; onUnsave: () => void; onAdd: (albumId: string, item: SavedItem) => void | Promise<void>; onRemove: (albumItemId: string) => void }) {
  ...
  const content = <><span className="mb-2 inline-flex rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-bold uppercase dark:bg-zinc-800">{label(type)}</span><h3 className="font-bold">{item.unavailable ? 'Post unavailable' : item.title ?? item.id}</h3>{item.description && <p className="mt-1 line-clamp-2 text-sm text-slate-500 dark:text-zinc-400">{item.description}</p>}<p className="mt-1.5 text-xs text-slate-400 dark:text-zinc-500">{[!item.unavailable && item.relatedName ? item.relatedName : null, !item.unavailable && item.siteName ? item.siteName : null, `Saved ${new Date(item.savedAt).toLocaleDateString()}`].filter(Boolean).join(' · ')}</p></>;
