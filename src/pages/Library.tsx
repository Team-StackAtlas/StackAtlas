...
import { Bookmark, ExternalLink, Folder, Plus, Search, Trash2 } from 'lucide-react';
...
type Filter = 'all' | 'dispatch' | 'signal' | 'source' | 'external_link' | 'album';

function label(type: string) {
  return type === 'dispatch' ? 'Dispatch' : type === 'signal' ? 'Signal' : type === 'source' ? 'Source' : type === 'external_link' ? 'External Link' : 'Album';
}

export default function Library() {
  const { status } = useAuth();
  const { savedItems, saveItem, unsaveItem } = useSaved();
  ...
  const resolved = savedItems.map((item) => {
    const post = getPosts().find((p) => p.id === item.id && p.type.toLowerCase() === item.type.toLowerCase());
    const source = SOURCES.find((s) => s.id === item.id);
    return { ...item, title: item.title ?? post?.title ?? source?.title, description: item.description ?? post?.content, url: item.url ?? source?.url, siteName: item.siteName ?? source?.publisher, unavailable: !post && !source && item.type !== 'external_link' };
  });
  ...
  const rows = useMemo(() => {
    ...
    return [...resolved, ...albumRows]
      ...
      .filter((item) => `${'title' in item ? item.title : ''} ${'description' in item ? item.description ?? '' : ''} ${'siteName' in item ? item.siteName ?? '' : ''}`.toLowerCase().includes(query.toLowerCase()))
  ...
  async function submitLink(event: FormEvent<HTMLFormElement>) {
    ...
    if (!url || !title) return;
    const id = url;
    saveItem(id, 'external_link', { title, url, description: String(form.get('description') ?? '').trim() || undefined, siteName: new URL(url).hostname });
    const albumId = String(form.get('albumId') ?? '');
    if (albumId) await addToAlbum(albumId, { itemId: id, itemType: 'external_link', title, url, description: String(form.get('description') ?? '').trim() || undefined });
    event.currentTarget.reset();
  }
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 pb-24 pt-6">
      ...
      <div className="mb-5 flex flex-wrap gap-2">{(['all','dispatch','signal','source','external_link','album'] as Filter[]).map((tab) => <button key={tab} onClick={() => setFilter(tab)} className={`rounded-full px-4 py-2 text-sm font-medium ${filter === tab ? 'bg-slate-900 text-white dark:bg-zinc-100 dark:text-zinc-950' : 'bg-white text-slate-600 dark:bg-zinc-900 dark:text-zinc-400'}`}>{tab === 'all' ? 'All saved' : label(tab)}</button>)}</div>
      <div className="mb-6 grid gap-4 md:grid-cols-2">
        <form onSubmit={submitLink} className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900"><h2 className="mb-3 flex items-center gap-2 font-bold"><Plus size={16}/> Save external link</h2><input name="url" type="url" required placeholder="https://example.com" className="mb-2 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 dark:border-zinc-800 dark:bg-zinc-950"/><input name="title" required placeholder="Title" className="mb-2 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 dark:border-zinc-800 dark:bg-zinc-950"/><textarea name="description" placeholder="Optional note" className="mb-2 h-20 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 dark:border-zinc-800 dark:bg-zinc-950"/><select name="albumId" className="mb-3 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 dark:border-zinc-800 dark:bg-zinc-950"><option value="">No album</option>{albums.map((a)=><option key={a.id} value={a.id}>{a.title}</option>)}</select><button className="rounded-xl bg-emerald-500 px-4 py-2 text-sm font-semibold text-white">Save link</button></form>
...
function SavedRow({ item, albums, albumItems, onUnsave, onAdd, onRemove }: { item: HookSavedItem & { unavailable?: boolean }; albums: { id: string; title: string }[]; albumItems: { id: string; albumId: string; savedItemType: SavedItemType; savedItemId: string }[]; onUnsave: () => void; onAdd: (albumId: string, item: SavedItem) => void | Promise<void>; onRemove: (albumItemId: string) => void }) {
  ...
  return <div className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900"><div className="flex justify-between gap-3"><div><span className="mb-2 inline-flex rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-bold uppercase dark:bg-zinc-800">{label(type)}</span><h3 className="font-bold">{item.unavailable ? `${label(type)} unavailable` : item.title ?? item.id}</h3>{item.description && <p className="mt-1 line-clamp-2 text-sm text-slate-500">{item.description}</p>}{linked && !item.unavailable && <a href={linked} className="mt-2 inline-flex items-center gap-1 text-sm text-emerald-600 hover:underline"><ExternalLink size={14}/> Open</a>}</div><button onClick={onUnsave} title="Unsave"><Bookmark className="fill-current text-emerald-500" size={18}/></button></div><div className="mt-3 flex flex-wrap items-center gap-2"><select onChange={(e) => e.target.value && onAdd(e.target.value, { itemId: item.id, itemType: type, title: item.title, url: item.url, description: item.description })} defaultValue="" className="rounded-lg border border-slate-200 bg-slate-50 px-2 py-1 text-xs dark:border-zinc-800 dark:bg-zinc-950"><option value="">Add to album…</option>{albums.map((album) => <option key={album.id} value={album.id}>{album.title}</option>)}</select>{current.map((albumItem) => <button key={albumItem.id} onClick={() => onRemove(albumItem.id)} className="rounded-full bg-slate-100 px-2 py-1 text-xs dark:bg-zinc-800">Remove from {albums.find((a) => a.id === albumItem.albumId)?.title ?? 'album'}</button>)}</div></div>;
}
