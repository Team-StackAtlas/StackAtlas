...
function SavedRow({ item, albums, albumItems, onUnsave, onAdd, onRemove }: { item: HookSavedItem & { unavailable?: boolean }; albums: { id: string; title: string }[]; albumItems: { id: string; albumId: string; savedItemType: SavedItemType; savedItemId: string }[]; onUnsave: () => void; onAdd: (albumId: string, item: SavedItem) => void | Promise<void>; onRemove: (albumItemId: string) => void }) {
  ...
  return <div className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900"><div className="flex justify-between gap-3"><div><span className="mb-2 inline-flex rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-bold uppercase dark:bg-zinc-800">{label(type)}</span><h3 className="font-bold">{item.unavailable ? 'Post unavailable' : item.title ?? item.id}</h3>{item.description && <p className="mt-1 line-clamp-2 text-sm text-slate-500">{item.description}</p>}{item.relatedName && !item.unavailable && <p className="mt-1 text-xs text-slate-500">Related: {item.relatedName}</p>}{item.siteName && !item.unavailable && <p className="mt-1 text-xs text-slate-500">Author/source: {item.siteName}</p>}{item.originalCreatedAt && !item.unavailable && <p className="mt-1 text-xs text-slate-500">Posted: {new Date(item.originalCreatedAt).toLocaleString()}</p>}<p className="mt-1 text-xs text-slate-500">Saved: {new Date(item.savedAt).toLocaleString()}</p>{linked && !item.unavailable && (type === 'dispatch' || type === 'signal' ? <Link to={linked} className="mt-2 inline-flex items-center gap-1 text-sm text-emerald-600 hover:underline"><ExternalLink size={14}/> Open</Link> : <a href={linked} className="mt-2 inline-flex items-center gap-1 text-sm text-emerald-600 hover:underline"><ExternalLink size={14}/> Open</a>)}</div><button onClick={onUnsave} title="Unsave"><Bookmark className="fill-current text-emerald-500" size={18}/></button></div><div className="mt-3 flex flex-wrap items-center gap-2"><select onChange={(e) => e.target.value && onAdd(e.target.value, { itemId: item.id, itemType: type, title: item.title, url: item.url, description: item.description })} defaultValue="" className="rounded-lg border border-slate-200 bg-slate-50 px-2 py-1 text-xs dark:border-zinc-800 dark:bg-zinc-950"><option value="">Add to album…</option>{albums.map((album) => <option key={album.id} value={album.id}>{album.title}</option>)}</select>{current.map((albumItem) => <button key={albumItem.id} onClick={() => onRemove(albumItem.id)} className="rounded-full bg-slate-100 px-2 py-1 text-xs dark:bg-zinc-800">Remove from {albums.find((a) => a.id === albumItem.albumId)?.title ?? 'album'}</button>)}</div></div>;
}
