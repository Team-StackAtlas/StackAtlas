...
import { COMMUNITY_CATEGORIES, getEntityCategories, type CommunityCategory } from '../data/communityTaxonomy';
...
export default function Square() {
  ...
  const [activePostType, setActivePostType] = useState<'All' | 'Dispatches' | 'Signals'>('All');
  const [activeCategory, setActiveCategory] = useState<string | 'All'>('All');
  const [activeMarker, setActiveMarker] = useState<string | null>(null);
  const [followingKinds, setFollowingKinds] = useState({ Users: true, Substances: true, Brands: true, Stacks: true });
  ...
  const { posts: allPosts, loading } = useCommunityPosts();
  ...
  const passesFeedFilters = (p: Post) => {
    ...
    if (feedType === 'Following') {
      const isLinkedToFollowed =
        (followingKinds.Users && isFollowing('user', p.author.id)) ||
        (followingKinds.Substances && !!p.supplementId && isFollowing('substance', p.supplementId)) ||
        (followingKinds.Stacks && !!p.stackId && isFollowing('stack', p.stackId)) ||
        (followingKinds.Brands && !!p.brandId && isFollowing('brand', p.brandId));
    ...
    if (typeParam === 'dispatches' || activePostType === 'Dispatches') {
      if (p.type !== 'Dispatch') return false;
    }
    if (typeParam === 'signals' || activePostType === 'Signals') {
      if (p.type !== 'Signal') return false;
    }
    if (activeCategory !== 'All') {
      const cats = getEntityCategories(p.supplementId ? 'substance' : p.stackId ? 'stack' : p.brandId ? 'brand' : null, p.supplementId ?? p.stackId ?? p.brandId ?? null);
      if (!cats.includes(activeCategory as CommunityCategory)) return false;
    }
    const q = searchText.trim().toLowerCase();
    if (q && ![p.title, p.content, p.author.username, ...(p.bearings ?? [])].join(' ').toLowerCase().includes(q)) return false;
    return true;
  };
    ...
    <div className="flex flex-col min-h-screen bg-slate-50 dark:bg-zinc-950 text-slate-900 dark:text-zinc-50 pb-24 md:pb-8 transition-colors duration-200">
      ...
      <div className="p-4 sticky top-[104px] md:top-[46px] z-30 bg-white/90 dark:bg-zinc-950/90 backdrop-blur-md border-b border-slate-200 dark:border-zinc-800 flex gap-3">
        <div className="relative flex-1">
          ...
          <input
            type="search"
            placeholder="Search posts, Bearings, or authors..."
            value={searchText}
      ...
      {!substanceId && !stackId && !brandId && !bearingParam && (
        <div className="px-4 pt-4 pb-2 space-y-2">
          <div className="flex overflow-x-auto hide-scrollbar gap-2 pb-2">
            {(['All', 'Dispatches', 'Signals'] as const).map(type => (
              <button key={type} onClick={() => setActivePostType(type)} className={cn(
                "whitespace-nowrap px-4 py-1.5 rounded-full text-sm font-medium transition-all border",
                activePostType === type ? "bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-200 dark:border-emerald-500/20 shadow-sm" : "bg-white dark:bg-zinc-900 text-slate-600 dark:text-zinc-400 border-slate-200 dark:border-zinc-800 hover:bg-slate-50 dark:hover:bg-zinc-800"
              )}>{type}</button>
            ))}
          </div>
          <div className="flex overflow-x-auto hide-scrollbar gap-2 pb-2">
            {categories.map(category => (
              <button key={category} onClick={() => setActiveCategory(category)} className={cn(
                "whitespace-nowrap px-3 py-1 rounded-lg text-xs font-medium transition-all border",
                activeCategory === category ? "bg-slate-100 dark:bg-zinc-800 text-slate-900 dark:text-zinc-200 border-slate-300 dark:border-zinc-700 shadow-sm" : "bg-white/50 dark:bg-zinc-900/50 text-slate-500 dark:text-zinc-500 border-slate-200 dark:border-zinc-800/50 hover:bg-slate-100 dark:hover:bg-zinc-800"
              )}>
                <span className="mr-2 inline-block h-3 w-3 rounded bg-slate-100 align-middle dark:bg-zinc-800" />
                {category}
              </button>
            ))}
          </div>
      ...
      {feedType === 'Following' && (
        <div className="px-4 pb-2">
          <div className="flex overflow-x-auto hide-scrollbar gap-2 pb-2">
            {Object.keys(followingKinds).map((kind) => (
              <button key={kind} onClick={() => setFollowingKinds(prev => ({ ...prev, [kind]: !prev[kind as keyof typeof prev] }))} className={cn(
                "whitespace-nowrap px-3 py-1 rounded-full text-xs font-medium transition-all border",
                followingKinds[kind as keyof typeof followingKinds] ? "bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-300 dark:border-emerald-500/20" : "bg-white/50 dark:bg-zinc-900/50 text-slate-500 dark:text-zinc-500 border-slate-200 dark:border-zinc-800/50"
              )}>{kind}</button>
            ))}
          </div>
        </div>
      ...
      <div className="flex-1 px-4 space-y-4 max-w-2xl mx-auto w-full">
        ...
        {loading && (
          <div className="rounded-xl border border-slate-200 bg-white/70 px-4 py-3 text-sm text-slate-500 shadow-sm dark:border-zinc-800 dark:bg-zinc-900/50 dark:text-zinc-400">Loading community posts…</div>
        )}
        {!loading && sortedPosts.length === 0 && (
          <div className="rounded-xl border border-slate-200 bg-white/70 px-4 py-8 text-center text-sm text-slate-500 shadow-sm dark:border-zinc-800 dark:bg-zinc-900/50 dark:text-zinc-400">No posts match these filters.</div>
        )}
