...
import { ArrowLeft, Check, HelpCircle, Search, X } from 'lucide-react';
...
type EntityOption = { id: string; name: string; type: EntityType; categories: string[]; aliases?: string[] };
...
const CURATED_COMPANION_SUBSTANCES: EntityOption[] = [
  { id: 'tb-500', name: 'TB-500', type: 'supplement', categories: ['Recovery', 'Pain & Injury'], aliases: ['TB-500', 'TB500', 'tb-500', 'tb 500'] },
];
...
const normalizeSubstanceKey = (value: string) => value.toLowerCase().replace(/[^a-z0-9]/g, '');

function getSubstanceOptions(): EntityOption[] {
  const substances = SUPPLEMENTS.map(s => ({
    id: s.id,
    name: s.name,
    type: 'supplement' as const,
    categories: s.paths.map(path => path.category),
    aliases: [s.id, s.name, ...(s.possiblePairings ?? [])],
  }));
  const existingKeys = new Set(substances.flatMap(substance => [substance.id, substance.name].map(normalizeSubstanceKey)));
  const curated = CURATED_COMPANION_SUBSTANCES.filter(substance => ![substance.id, substance.name, ...(substance.aliases ?? [])].some(alias => existingKeys.has(normalizeSubstanceKey(alias))));
  return [...substances, ...curated].sort((a, b) => a.name.localeCompare(b.name));
}

function getEntityOptions(kind: 'dispatch' | 'signal'): EntityOption[] {
  const substances = getSubstanceOptions();
  if (kind === 'dispatch') return substances;
...
function getEntityCategory(entity: EntityOption | null) {
  ...
  return SUPPLEMENTS.find(s => s.id === entity.id)?.paths[0]?.category ?? entity.categories[0] ?? 'General';
}
...
function getSubstanceCompanionKey(entity: EntityOption) {
  const aliases = [entity.id, entity.name, ...(entity.aliases ?? [])].map(normalizeSubstanceKey);
  if (aliases.includes('bpc157')) return 'bpc157';
  if (aliases.includes('tb500')) return 'tb500';
  if (aliases.includes('caffeine')) return 'caffeine';
  if (aliases.includes('ltheanine')) return 'ltheanine';
  return aliases[0];
}

function getCompanionOptions(entity: EntityOption | null) {
  if (!entity || entity.type !== 'supplement') return [];
  const entityKey = getSubstanceCompanionKey(entity);
  const companionKeys = CURATED_COMPANION_PAIRS.flatMap(([a, b]) => {
    if (entityKey === a) return [b];
    if (entityKey === b) return [a];
    return [];
  });
  const substances = getSubstanceOptions();
  return companionKeys
    .map(key => substances.find(substance => getSubstanceCompanionKey(substance) === key))
    .filter((substance): substance is EntityOption => Boolean(substance));
}
...
function CreateTypeCard({ title, description, tooltip, tone, onClick }: {
  title: CreateType;
  description: string;
  tooltip: string;
  tone: 'emerald' | 'blue';
  ...
  const toneClasses = tone === 'emerald'
    ? 'hover:border-emerald-500 hover:shadow-emerald-500/10 focus-visible:border-emerald-500 dark:hover:border-emerald-500 dark:focus-visible:border-emerald-500 group-hover:text-emerald-600 group-focus-visible:text-emerald-600 dark:group-hover:text-emerald-400 dark:group-focus-visible:text-emerald-400'
    : 'hover:border-blue-500 hover:shadow-blue-500/10 focus-visible:border-blue-500 dark:hover:border-blue-500 dark:focus-visible:border-blue-500 group-hover:text-blue-600 group-focus-visible:text-blue-600 dark:group-hover:text-blue-400 dark:group-focus-visible:text-blue-400';

  return (
    <button type="button" onClick={onClick} className={cn('group flex flex-1 flex-col items-center justify-center rounded-3xl border-2 border-slate-200 bg-white p-8 text-center transition-all hover:shadow-lg focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 dark:border-zinc-800 dark:bg-zinc-900 dark:focus-visible:ring-offset-zinc-950', toneClasses)}>
      <span className="mb-4 inline-flex items-center gap-2 text-2xl font-bold text-slate-800 transition-colors dark:text-zinc-100">
        {title}
        <span className="relative inline-flex" aria-label={`${title} details`}>
          <HelpCircle size={16} aria-hidden="true" className="text-slate-400 transition-colors group-hover:text-current group-focus-visible:text-current" />
          <span role="tooltip" className="pointer-events-none absolute left-1/2 top-full z-20 mt-2 hidden w-72 -translate-x-1/2 rounded-xl border border-slate-200 bg-white p-3 text-left text-xs font-medium leading-relaxed text-slate-600 shadow-xl group-hover:block group-focus-visible:block dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-300">
            {tooltip}
          </span>
        </span>
      </span>
      <span className="text-sm leading-relaxed text-slate-500 dark:text-zinc-400">{description}</span>
    </button>
  );
}
...
function DurationField({ value, onChange }: { value: Duration; onChange: (value: Duration) => void }) {
  return (
    <div>
      ...
      <div className="inline-grid max-w-xs grid-cols-[7rem_auto] gap-2">
...
function BearingPicker({ mode, selected, entity, onChange, error }: {
  ...
  const defaultGroupName = suggested.length > 0
    ? groupList.find(group => group.bearings.some(bearing => suggested.includes(bearing)))?.name ?? groupList[0]?.name
    : groupList[0]?.name;
  const [activeGroupName, setActiveGroupName] = useState(defaultGroupName ?? '');
  const inlineResults = query.trim()
    ? allowed.filter(bearing => bearing.toLowerCase().includes(query.toLowerCase())).slice(0, 10)
    : suggested.slice(0, 10);
  const activeGroup = groupList.find(group => group.name === activeGroupName) ?? groupList[0];
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <div className="mt-3">
          ...
          {inlineResults.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {inlineResults.map(renderBearingButton)}
            </div>
          ) : (
            <p className="text-sm text-slate-500 dark:text-zinc-400">Search or browse all Bearings.</p>
          )}
        </div>
        <button type="button" onClick={() => { setIsBrowsingAll(true); setModalQuery(''); }} className="mt-3 text-sm font-semibold text-emerald-600 hover:underline dark:text-emerald-400">Browse all Bearings</button>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4">
          <div className="flex max-h-[85vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl bg-white shadow-xl dark:bg-zinc-900">
            <div className="sticky top-0 z-10 border-b border-slate-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900">
              ...
              <div className="relative">
                <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                <input value={modalQuery} onChange={e => setModalQuery(e.target.value)} placeholder="Search all Bearings..." className="w-full rounded-xl border border-slate-200 bg-transparent py-2.5 pl-10 pr-3 text-sm focus:border-emerald-500 focus:outline-none dark:border-zinc-700" />
              </div>
            ...
            <div className="grid min-h-0 flex-1 overflow-hidden md:grid-cols-[220px_1fr]">
              {!modalQuery.trim() && (
                <aside className="border-b border-slate-200 p-4 dark:border-zinc-800 md:overflow-auto md:border-b-0 md:border-r">
                  {suggested.length > 0 && (
                    <button type="button" onClick={() => setActiveGroupName('Suggested')} className={cn('mb-2 w-full rounded-xl px-3 py-2 text-left text-sm font-semibold', activeGroupName === 'Suggested' ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300' : 'text-slate-600 hover:bg-slate-50 dark:text-zinc-300 dark:hover:bg-zinc-800')}>Suggested</button>
                  )}
                  {groupList.map(group => (
                    <button key={group.name} type="button" onClick={() => setActiveGroupName(group.name)} className={cn('mb-1 w-full rounded-xl px-3 py-2 text-left text-sm font-medium', activeGroupName === group.name ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300' : 'text-slate-600 hover:bg-slate-50 dark:text-zinc-300 dark:hover:bg-zinc-800')}>
                      {group.name}
                    </button>
                  ))}
                </aside>
              )}
              <main className={cn('overflow-auto p-5', modalQuery.trim() && 'md:col-span-2')}>
                {modalQuery.trim() ? (
                  modalSearchGroups.length > 0 ? (
                    <div className="space-y-5">
                      {modalSearchGroups.map(group => (
                        <section key={group.name}>
                          <h4 className="mb-2 text-sm font-semibold text-slate-700 dark:text-zinc-300">{group.name}</h4>
                          <div className="flex flex-wrap gap-2">{group.bearings.map(renderBearingButton)}</div>
                        </section>
                      ))}
                    </div>
                  ) : <p className="text-sm text-slate-500 dark:text-zinc-400">No Bearings match that search.</p>
                ) : activeGroupName === 'Suggested' && suggested.length > 0 ? (
                  <section>
                    <h4 className="mb-2 text-sm font-semibold text-slate-700 dark:text-zinc-300">Suggested</h4>
                    <div className="flex flex-wrap gap-2">{suggested.map(renderBearingButton)}</div>
                  </section>
                ) : activeGroup ? (
                  <section>
                    <h4 className="mb-2 text-sm font-semibold text-slate-700 dark:text-zinc-300">{activeGroup.name}</h4>
                    <div className="flex flex-wrap gap-2">{activeGroup.bearings.map(renderBearingButton)}</div>
                  </section>
                ) : null}
              </main>
            </div>
            <div className="border-t border-slate-200 bg-white p-4 text-right dark:border-zinc-800 dark:bg-zinc-900">
      ...
      <div className="mx-auto w-full max-w-4xl p-4 sm:p-6">
        {!activeType ? (
          <div className="mx-auto mt-10 flex min-h-[50vh] max-w-2xl flex-col items-stretch justify-center gap-6 sm:flex-row">
            <CreateTypeCard
              title="Dispatch"
              description="Log a structured experience with what you took, how much, how often, and what happened."
              tooltip="Dispatches help StackAtlas turn real user experiences into useful community data. Use them when you can share what you took, dose, frequency, duration, and enough context for others to compare."
              tone="emerald"
              onClick={() => setActiveType('Dispatch')}
            />
            <CreateTypeCard
              title="Signal"
              description="Ask a question, share an observation, or start a discussion."
              tooltip="Signals are freeform community posts. They can be general, or linked to a substance, brand, or stack if you want the post connected to something specific."
              tone="blue"
