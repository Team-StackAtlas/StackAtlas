...
export function createSupabaseAccountServices(client: SupabaseClient): {
  ...
  const saved: SavedService = {
    async list(userId: ID) {
      ...
      if (error) throw error;
      return (data ?? []).map((r: any) => ({ itemType: r.item_type, itemId: r.item_id, savedAt: r.created_at, title: r.title ?? undefined, url: r.url ?? undefined, description: r.description ?? undefined, siteName: r.site_name ?? undefined, relatedType: r.related_type ?? undefined, relatedId: r.related_id ?? undefined, relatedName: r.related_name ?? undefined, authorName: r.author_name ?? undefined, itemCreatedAt: r.item_created_at ?? undefined }) as SavedItem);
    },
    async add(userId: ID, item: SavedItem) {
      const payload = { user_id: userId, item_type: item.itemType, item_id: item.itemId, title: item.title ?? null, url: item.url ?? null, description: item.description ?? null, site_name: item.siteName ?? null, related_type: item.relatedType ?? null, related_id: item.relatedId ?? null, related_name: item.relatedName ?? null, author_name: item.authorName ?? null, item_created_at: item.itemCreatedAt ?? null };
      const { error } = await client.from('saved_items').upsert(payload);
      if (!error) return;
